# MarketAdjuster – Windows Installer (Desktop shortcut that opens browser reliably)
...
if (-not $pyInfo) {
    Write-Host "Python is not installed. Install Python 3.10+ and re-run." -ForegroundColor Red
    exit 1
}
...
$launcherBat = Join-Path $installRoot "Run MarketAdjuster.bat"

# Use fixed port so the shortcut always points to the right place
$port = 8501
$address = "127.0.0.1"
...
# Start Streamlit in a separate process so we can reliably open the browser AFTER it binds the port.
`$args = @('-m','streamlit','run',`"$app`",'--server.port',`"$port`",'--server.address',`"$address`",'--server.headless','false')
`$p = Start-Process -FilePath `$venvPy -ArgumentList `$args -PassThru -WindowStyle Normal

# Wait for the port to open (up to ~20s)
for (`$i=0; `$i -lt 40; `$i++) {
  try {
...
# Open the browser (fallback chain)
try {
...
# Keep this launcher window alive as long as Streamlit is alive
...
$shortcutPath = Join-Path $desktop "MarketAdjuster.lnk"

# Prefer repo-provided icon if present
...
Write-Host "Install complete. Launch using the 'MarketAdjuster' desktop icon." -ForegroundColor Cyan
